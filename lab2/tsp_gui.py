...
import random
import time
from tsp_solver import simulated_annealing, nearest_neighbor

class TSPInteractiveGUI:
    def __init__(self, master):
        self.master = master
        master.title("Интерактивная задача коммивояжера (Отжиг)")
    ...
    def create_control_frame(self, master):
        ...
        general_frame.grid(row=0, column=0, padx=5, pady=5, sticky="ew") #Use grid

        run_button = ttk.Button(general_frame, text="Найти кратчайший путь (Отжиг)", command=self.run_simulated_annealing)
        run_button.grid(row=0, column=0, padx=5, pady=2) #Use grid

        clear_button = ttk.Button(general_frame, text="Очистить поле", command=self.clear_graph)
        clear_button.grid(row=0, column=1, padx=5, pady=2) #Use grid

        delete_vertex_button = ttk.Button(general_frame, text="Удалить последнюю вершину", command=self.delete_last_vertex)
        delete_vertex_button.grid(row=1, column=0, padx=5, pady=2) #Use grid

        delete_path_button = ttk.Button(general_frame, text="Удалить последнее ребро", command=self.delete_last_edge)
        delete_path_button.grid(row=1, column=1, padx=5, pady=2) #Use grid

        start_vertex_frame = ttk.LabelFrame(control_frame, text="Начальная вершина")
        start_vertex_frame.grid(row=1, column=0, padx=5, pady=5, sticky="ew") #Use grid

        self.start_vertex_label = ttk.Label(start_vertex_frame, text="Начальная вершина:")
        self.start_vertex_label.grid(row=0, column=0, padx=5, pady=2) #Use grid

        self.start_vertex_entry = ttk.Entry(start_vertex_frame, width=5)
        self.start_vertex_entry.grid(row=0, column=1, padx=5, pady=2) #Use grid

        self.set_start_vertex_button = ttk.Button(start_vertex_frame, text="Установить", command=self.set_start_vertex)
        self.set_start_vertex_button.grid(row=0, column=2, padx=5, pady=2) #Use grid

        load_matrix_frame = ttk.LabelFrame(control_frame, text="Загрузка матрицы")
        load_matrix_frame.grid(row=2, column=0, padx=5, pady=5, sticky="ew") #Use grid

        load_matrix_button = ttk.Button(load_matrix_frame, text="Загрузить матрицу из таблицы", command=self.load_matrix_from_table)
        load_matrix_button.grid(row=0, column=0, padx=5, pady=2) #Use grid

        edge_weight_frame = ttk.LabelFrame(control_frame, text="Вес ребра")
        edge_weight_frame.grid(row=3, column=0, padx=5, pady=5, sticky="ew") #Use grid

        ttk.Label(edge_weight_frame, text="Ребро:").grid(row=0, column=0, padx=5, pady=2) #Use grid
        self.edge_entry = ttk.Entry(edge_weight_frame, width=5)
        self.edge_entry.grid(row=0, column=1, padx=5, pady=2) #Use grid

        ttk.Label(edge_weight_frame, text="Вес ребра:").grid(row=1, column=0, padx=5, pady=2) #Use grid
        self.edge_weight_entry = ttk.Entry(edge_weight_frame, width=5)
        self.edge_weight_entry.grid(row=1, column=1, padx=5, pady=2) #Use grid

        self.update_edge_weight_button = ttk.Button(edge_weight_frame, text="Изменить", command=self.update_edge_weight)
        self.update_edge_weight_button.grid(row=1, column=2, padx=5, pady=2) #Use grid

        sa_params_frame = ttk.LabelFrame(control_frame, text="Параметры отжига")
        sa_params_frame.grid(row=0, column=1, rowspan=4, padx=5, pady=5, sticky="nsew") #Use grid

        ttk.Label(sa_params_frame, text="T init:").grid(row=0, column=0, padx=5, pady=2) #Use grid
        self.initial_temperature_entry = ttk.Entry(sa_params_frame, width=5)
        self.initial_temperature_entry.insert(0, str(self.initial_temperature))
        self.initial_temperature_entry.grid(row=0, column=1, padx=5, pady=2) #Use grid

        ttk.Label(sa_params_frame, text="Cooling rate:").grid(row=1, column=0, padx=5, pady=2) #Use grid
        self.cooling_rate_entry = ttk.Entry(sa_params_frame, width=5)
        self.cooling_rate_entry.insert(0, str(self.cooling_rate))
        self.cooling_rate_entry.grid(row=1, column=1, padx=5, pady=2) #Use grid

        ttk.Label(sa_params_frame, text="Iterations:").grid(row=2, column=0, padx=5, pady=2) #Use grid
        self.num_iterations_entry = ttk.Entry(sa_params_frame, width=5)
        self.num_iterations_entry.insert(0, str(self.num_iterations))
        self.num_iterations_entry.grid(row=2, column=1, padx=5, pady=2) #Use grid

        self.modification_var = tk.BooleanVar()
        modification_check = ttk.Checkbutton(sa_params_frame, text="Модификация", variable=self.modification_var)
        modification_check.grid(row=3, column=0, columnspan=2, padx=5, pady=2) #Use grid

        control_frame.columnconfigure(0, weight=1)
        control_frame.columnconfigure(1, weight=1)

        sa_params_frame.columnconfigure(0, weight=1)
    ...
    def get_sa_parameters(self):
        """Gets simulated annealing parameters from entry fields."""
        try:
            self.initial_temperature = float(self.initial_temperature_entry.get())
            self.cooling_rate = float(self.cooling_rate_entry.get())
            self.num_iterations = int(self.num_iterations_entry.get())

            if self.initial_temperature <= 0 or self.cooling_rate <= 0 or self.cooling_rate >= 1 or self.num_iterations <= 0:
                messagebox.showerror("Ошибка", "Параметры отжига должны быть положительными. Охлаждение должно быть от 0 до 1.")
                return False

            return True
        except ValueError:
            messagebox.showerror("Ошибка", "Пожалуйста, введите числовые значения для параметров отжига.")
            return False

    def run_simulated_annealing(self):
        """Runs simulated annealing to find the TSP solution."""
        if not self.get_sa_parameters():
            return
